...
Map<ext_char, int> getFrequencyTable(istream& file) {
	Map<ext_char, int> myTable;
    char ch;
	while(file.get(ch)){
		if(!myTable.containsKey(ch)){
			myTable.put(ch, 1);
		}
		else{ 
			myTable.put(ch, myTable.get(ch)+1);
		}
	}
	
    myTable.put(PSEUDO_EOF, 1);
	return myTable;
}
...
Node* buildEncodingTree(Map<ext_char, int>& frequencies) {
	...
	foreach(ext_char ch in frequencies){
		...
		prQ.enqueue(node, node->weight);
	}
	while(prQ.size()!=1){
		Node* child1 = prQ.dequeue();
		Node* child2 = prQ.dequeue();
		Node* parent = new Node;
		parent->zero = child1;
		parent->one = child2;
		parent->character = NOT_A_CHAR;
		parent->weight = child1->weight + child2->weight;
		prQ.enqueue(parent, parent->weight);
	}
	return prQ.dequeue();
}
 ...
 */ 
void encodeMap(Node* root, Map<ext_char, string>& map, string curr) {
    if (!root) {
        return;
    }
    if (root->character != NOT_A_CHAR) {
        map.put(root->character, curr);
		return ;
        
    }
	 
     encodeMap(root->zero, map, curr + '0');
     encodeMap(root->one, map, curr + '1');
}
...
void encodeFile(istream& infile, Node* encodingTree, obstream& outfile) {
    ...
    encodeMap(encodingTree, encodingMap, "");

    char character;
    while (infile.get(character)) {
        string bits = encodingMap[character];
        foreach (char ch in bits) {
            outfile.writeBit(ch - '0');
        }
    }

    string bits = encodingMap[PSEUDO_EOF];
    foreach (char ch in bits) {
        outfile.writeBit(ch - '0');
    }
}
